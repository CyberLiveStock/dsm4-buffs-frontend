...
export default function Equipe() {
  ...
  const filteredUsers = Array.isArray(users)
    ? users.filter((user) => {
        ...
        const matchesCargo =
          cargoFilter === "todos" || user.cargo === cargoFilter;
        return matchesSearch && matchesCargo;
      })
    : [];

  // Obter cargos únicos para o filtro
  const uniqueCargos = Array.isArray(users)
    ? [...new Set(users.map((u) => u.cargo).filter(Boolean))]
    : [];
      ...
      <div className="w-full max-w-[1200px] flex flex-col bg-white rounded-xl p-5 gap-4 box-border border border-[#e0e0e0] shadow-sm">
        <div>
          ...
          <p className="text-gray-600">
            Cadastre e gerencie o acesso dos funcionários ao sistema. Total:{" "}
            {Array.isArray(users) ? users.length : 0} funcionários
          </p>
        </div>
...
function AddUserModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.userName.trim()) return alert("Usuário é obrigatório");
    if (!formData.email.trim()) return alert("E-mail é obrigatório");
    if (!formData.password.trim()) return alert("Senha é obrigatória");
    if (!formData.cargo) return alert("Cargo é obrigatório");
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              ...
              >
                ...
                <option value="Auxiliar de Produção">
