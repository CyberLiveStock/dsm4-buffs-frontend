...
import { createProduction, updateProduction, deleteProduction } from "@/services/productionService"
import { fetchProductionStats, formatCurrency, formatDate, getStatusColor, getStatusText } from "@/utils/productionUtil"
...
export default function Producao() {
  ...
  const filteredColetas = allColetas.filter((coleta) => {
    const matchesSearch = coleta.empresaColeta?.toLowerCase().includes(searchTerm.toLowerCase()) || false
    const matchesStatus = statusFilter === "todos" || coleta.resultado === statusFilter
    return matchesSearch && matchesStatus
  })

  // Obter status únicos para o filtro
  const uniqueStatus = [...new Set(allColetas.map((c) => c.resultado).filter(Boolean))]
  ...
  const handleSaveEdit = async (formData) => {
    try {
      const success = await updateProduction(selectedColeta.productionId, formData)
      if (success) {
        await loadProductionData()
  ...
  return (
    <div className="w-full bg-[#f5f5f5] p-5 flex flex-col items-center gap-5 box-border">
      ...
      <div className="w-full max-w-[1200px] flex flex-col bg-white rounded-xl p-5 gap-4 box-border border border-[#e0e0e0] shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="bg-white p-4 rounded-lg shadow border border-[#e0e0e0]">
            ...
            <p className="text-2xl font-bold text-gray-800">{(stats.totalProduzido || 0).toFixed(1)} L</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow border border-[#e0e0e0]">
            ...
            <p className="text-2xl font-bold text-gray-800">{(stats.totalRetirado || 0).toFixed(1)} L</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow border border-[#e0e0e0]">
            ...
            <p className="text-2xl font-bold text-gray-800">{(stats.taxaAprovacao || 0).toFixed(1)}%</p>
          ...
          <div className="bg-white p-4 rounded-lg shadow border border-[#e0e0e0]">
            ...
            <p className="text-2xl font-bold text-gray-800">{(stats.totalRejeitado || 0).toFixed(1)} L</p>
      ...
      <div className="w-full max-w-[1200px] flex flex-col bg-white rounded-xl p-5 gap-4 box-border border border-[#e0e0e0] shadow-sm">
        ...
        <div className="flex gap-4 flex-wrap items-end">
          <div className="flex flex-col">
            <label className="font-semibold mb-1.5 text-gray-700 text-sm">Buscar por Empresa</label>
            <input
              type="text"
            ...
            <tbody>
              {filteredColetas.length === 0 ? (
                ...
                filteredColetas.map((coleta, index) => (
                  <tr key={`${coleta.productionId}-${index}`} className={index % 2 === 0 ? "bg-[#fafafa]" : "bg-white"}>
                    <td className="p-3 text-gray-800 text-base">{formatDate(coleta.dataColeta)}</td>
                    <td className="p-3 text-gray-800 text-base">{coleta.empresaColeta || "Não informado"}</td>
                    <td className="p-3 text-gray-800 text-base">{(coleta.quantidadeColetada || 0).toFixed(1)} L</td>
                    <td className="p-3 text-gray-800 text-base">
                      {formatCurrency(Number.parseFloat(coleta.valorPago?.replace(",", ".") || 0))}
                    </td>
                    <td className="p-3 text-center text-base">
                      <span
                        className={`px-2.5 py-1.5 rounded-full text-sm font-bold inline-block w-20 ${getStatusColor(coleta.resultado)}`}
                      >
      ...
      <AddColetaModal isOpen={showAddColetaModal} onClose={() => setShowAddColetaModal(false)} onSave={handleSaveNew} />
...
function AddColetaModal({ isOpen, onClose, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.quantidadeColetada || Number(formData.quantidadeColetada) <= 0) {
    ...
    try {
      const dataToSend = {
        coletas: {
          ...
          valorPago: formData.valorPago.trim() || "0",
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-[1000]">
      <div className="bg-white p-6 rounded-lg w-[90%] max-w-[600px] flex flex-col gap-3 relative max-h-[90vh] overflow-y-auto">
        ...
        <h2 className="text-xl font-bold mb-2 text-gray-800">Registrar Nova Coleta</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, quantidadeColetada: e.target.value })}
                required
                min="0.1"
                step="0.1"
                disabled={isSubmitting}
            ...
            <div className="flex flex-col">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, empresaColeta: e.target.value })}
                required
                disabled={isSubmitting}
              />
            </div>
            ...
            <div className="flex flex-col">
              <label className="font-semibold mb-1.5 text-gray-700 text-sm">Valor Pago (R$)</label>
              <input
                type="text"
                className="py-2 px-3 border-2 border-[#D9DBDB] rounded-lg text-sm"
    ...
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-[1000]">
      <div className="bg-white p-6 rounded-lg w-[90%] max-w-[600px] flex flex-col gap-3 relative max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col">
              <label className="font-semibold mb-1.5 text-gray-700 text-sm">Data da Coleta</label>
              <input
                type="date"
                className="py-2 px-3 border-2 border-[#D9DBDB] rounded-lg text-sm"
                value={formData.dataColeta}
            ...
            <div className="flex flex-col">
              <label className="font-semibold mb-1.5 text-gray-700 text-sm">Quantidade Coletada (L)</label>
              <input
                type="number"
                className="py-2 px-3 border-2 border-[#D9DBDB] rounded-lg text-sm"
                value={formData.quantidadeColetada}
            ...
            <div className="flex flex-col">
              <label className="font-semibold mb-1.5 text-gray-700 text-sm">Empresa de Coleta</label>
              <input
                type="text"
                className="py-2 px-3 border-2 border-[#D9DBDB] rounded-lg text-sm"
                value={formData.empresaColeta}
            ...
            <div className="flex flex-col">
              <label className="font-semibold mb-1.5 text-gray-700 text-sm">Resultado da Análise</label>
              <select
                className="py-2 px-3 border-2 border-[#D9DBDB] rounded-lg text-sm text-gray-700"
                value={formData.resultado}
                onChange={(e) => setFormData({ ...formData, resultado: e.target.value })}
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex justify-center items-center z-[1000]">
      <div className="bg-white p-6 rounded-lg w-[90%] max-w-[500px] flex flex-col gap-4 relative">
        ...
        <h2 className="text-xl font-bold mb-2 text-gray-800">Detalhes da Coleta</h2>

        <div className="space-y-4">
          <div>
            <label className="font-semibold text-gray-700 text-sm block mb-1">Data da Coleta</label>
            <p className="text-gray-800 bg-gray-50 p-2 rounded border">{formatDate(coleta.dataColeta)}</p>
          </div>

          <div>
            <label className="font-semibold text-gray-700 text-sm block mb-1">Empresa de Coleta</label>
            <p className="text-gray-800 bg-gray-50 p-2 rounded border">{coleta.empresaColeta || "Não informado"}</p>
          </div>

          <div>
            <label className="font-semibold text-gray-700 text-sm block mb-1">Quantidade Coletada</label>
          ...
          <div>
            <label className="font-semibold text-gray-700 text-sm block mb-1">Valor Pago</label>
            <p className="text-gray-800 bg-gray-50 p-2 rounded border">
              {formatCurrency(Number.parseFloat(coleta.valorPago?.replace(",", ".") || 0))}
            </p>
          </div>
          ...
          <div>
            <label className="font-semibold text-gray-700 text-sm block mb-1">Resultado da Análise</label>
            <span
              className={`px-2.5 py-1 rounded-full text-xs font-bold inline-block ${getStatusColor(coleta.resultado)}`}
            >
              {getStatusText(coleta.resultado)}
            </span>
          </div>
          ...
          {coleta.desc && (
            <div>
              <label className="font-semibold text-gray-700 text-sm block mb-1">Observações</label>
              <p className="text-gray-800 bg-gray-50 p-2 rounded border">{coleta.desc}</p>
            </div>
          )}
        </div>
